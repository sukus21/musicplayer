...
function frame_read(p_buffer, p_previous) {
	...
	//Get sideinfo
	//show_debug_message("\nsideinfo: " + string(buffer_tell(p_buffer)));
	...
	//Read main data
	//show_debug_message("\nmaindata: " + string(buffer_tell(p_buffer)));
...
function frame_decode(p_frame) {
	...
	var _out = buffer_create(frameheader_bytesperframe(p_frame.header), buffer_fixed, 2);
...
function frame_requantize_long(p_frame, gr, ch, p_pos, p_sfb) {
	...
	static powtab = frame_init_powtab();
	...
	var _temp2 = powtab[abs(p_frame.maindata.is[gr][ch][p_pos])] * sign(p_frame.maindata.is[gr][ch][p_pos]);
...
function frame_print_is(p_frame) {
	var _out = "[ ";
	for(var i = 0; i < array_length(p_frame.maindata.is); i++) {
		for(var j = 0; j < array_length(p_frame.maindata.is[i]); j++) {
			var _sum = 0;
			for(var k = 0; k < array_length(p_frame.maindata.is[i][j]); k++)
				_sum += p_frame.maindata.is[i][j][k];
			
			_out += string_format(_sum, 4, 10) + " ";
		}
	}
